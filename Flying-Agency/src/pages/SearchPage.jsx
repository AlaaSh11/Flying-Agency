...
const RESULTS = [
  { airline: 'Emirates', dep: '08:30', arr: '10:45', from: 'BEY', to: 'DXB', dur: '2h 15m', stops: 'Nonstop', price: 340, stars: 5, badge: 'Best Value' },
  { airline: 'Air France', dep: '11:00', arr: '14:30', from: 'BEY', to: 'CDG', dur: '4h 30m', stops: 'Nonstop', price: 680, stars: 5, badge: 'Popular' },
  { airline: 'Turkish Airlines', dep: '14:15', arr: '16:10', from: 'BEY', to: 'IST', dur: '1h 55m', stops: 'Nonstop', price: 290, stars: 4, badge: 'Cheapest' },
  { airline: 'Qatar Airways', dep: '21:30', arr: '23:00', from: 'BEY', to: 'DOH', dur: '1h 30m', stops: 'Nonstop', price: 310, stars: 5, badge: '5 Star' },
];
      ...
      <div style={{ maxWidth: 1060, margin: '0 auto', padding: '0 5vw', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ background: t.bgCard, borderRadius: 24, padding: 28, border: `1px solid ${t.border}`, marginBottom: 26, boxShadow: `0 8px 40px ${t.glow}`, animation: 'fadeUp 0.5s ease 0.1s both' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
            ...
            <Btn onClick={() => { setLoading(true); setTimeout(() => { setLoading(false); setSearched(true); }, 1400); }} t={t}>Search Flights ✦</Btn>
          </div>
        </div>
        {loading && (
        ...
        {searched && !loading && (
          <div style={{ animation: 'fadeUp 0.5s ease both' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18, flexWrap: 'wrap', gap: 10 }}>
              <h3 style={{ color: t.text, fontWeight: 600, fontSize: 15 }}>{RESULTS.length} flights found</h3>
              ...
              {RESULTS.map((r, i) => (
                <div key={i} onClick={() => setSel(sel === i ? null : i)} data-h
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '160px 1fr auto auto', alignItems: 'center', gap: 20 }}>
                    <div>
                      <div style={{ fontWeight: 700, fontSize: 14, color: t.text, marginBottom: 6 }}>{r.airline}</div>
                      <span style={{ padding: '2px 10px', borderRadius: 20, fontSize: 9, fontWeight: 700, background: t.badge, color: t.a, fontFamily: "'Space Mono',monospace", letterSpacing: 0.5 }}>{r.badge}</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontFamily: "'Space Mono',monospace", fontSize: '1.2rem', fontWeight: 700, color: t.text }}>{r.dep}</div>
                        <div style={{ fontSize: 10, color: t.a, fontWeight: 700, fontFamily: "'Space Mono',monospace" }}>{r.from}</div>
                      </div>
                      <div style={{ flex: 1, textAlign: 'center' }}>
                        <div style={{ fontSize: 9, color: t.textMuted, marginBottom: 6, fontFamily: "'Space Mono',monospace" }}>{r.dur}</div>
                        ...
                        <div style={{ fontSize: 9, color: t.textMuted, marginTop: 6 }}>{r.stops}</div>
                      </div>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontFamily: "'Space Mono',monospace", fontSize: '1.2rem', fontWeight: 700, color: t.text }}>{r.arr}</div>
                        <div style={{ fontSize: 10, color: t.a, fontWeight: 700, fontFamily: "'Space Mono',monospace" }}>{r.to}</div>
                      </div>
                    </div>
                    <div style={{ textAlign: 'right', minWidth: 82 }}>
                      ...
                      <Btn t={t} sm sx={{ padding: '6px 16px', fontSize: 12 }}>Select</Btn>
                    </div>
                  </div>
                  {sel === i && (
